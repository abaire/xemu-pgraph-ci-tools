...
def _macos_extract_app(archive_file: str, target_app_bundle: str) -> None:
    ...
    try:
        with zipfile.ZipFile(archive_file, "r") as zip_ref:
            ...
            for file_info in zip_ref.infolist():
                if (
                    file_info.filename.startswith("xemu.app/")
            ...
            if not os.path.isfile(
                os.path.join(
                    app_bundle_directory, "xemu.app", "Contents", "MacOS", "xemu"
                )
            ):
            ...
            for file_info in zip_ref.infolist():
                if file_info.filename == "xemu.exe":
                    ...
                    if os.path.basename(target_executable) != "xemu.exe":
                        os.rename(
                            os.path.join(target_dir, "xemu.exe"), target_executable
                        )
                    return
    ...
    if system == "Linux":
        ...
        def check_asset(asset_name: str) -> bool:
            return (
                ...
                or asset_name.endswith("-macos-universal-unsigned.zip")
            )
    elif system == "Windows":

        def check_asset(asset_name: str) -> bool:
            if not asset_name.startswith("xemu-win-") or not asset_name.endswith(
                "release.zip"
            ):
                return False
...
def _generate_xemu_toml(
    ...
    *,
    ...
    bootrom_path = (
        os.path.abspath(os.path.expanduser(bootrom_path)).replace("\\", "/")
        if bootrom_path
        else ""
    )
    flashrom_path = (
        os.path.abspath(os.path.expanduser(flashrom_path)).replace("\\", "/")
        if flashrom_path
        else ""
    )
    eeprom_path = (
        os.path.abspath(os.path.expanduser(eeprom_path)).replace("\\", "/")
        if eeprom_path
        else ""
    )
    hdd_path = (
        os.path.abspath(os.path.expanduser(hdd_path)).replace("\\", "/")
        if hdd_path
        else ""
    )
...
def _build_macos_xemu_binary_paths(xemu_path: str) -> tuple[str, str]:
    ...
    if xemu_path.endswith(".app") and os.path.isdir(xemu_path):
        ...
        candidates = [
            os.path.join(
                os.path.dirname(xemu_path), "..", "dist", "xemu.app", "Contents"
            ),
    ...
    if contents_path and os.path.isdir(contents_path):
        library_path = os.path.join(
            contents_path, "Libraries", platform.uname().machine
        )
        if os.path.isdir(library_path):
            ...
            if library_path not in existing.split(":"):
                ...
                logger.debug(
                    "Set DYLD_FALLBACK_LIBRARY_PATH to %s",
                    os.environ["DYLD_FALLBACK_LIBRARY_PATH"],
                )

    if (
        "DYLD_FALLBACK_LIBRARY_PATH" not in os.environ
        or not os.environ["DYLD_FALLBACK_LIBRARY_PATH"]
    ):
                ...
                for root, _, _ in os.walk(cache_candidate):
                    if root.endswith(os.path.join("Contents", "Libraries", arch)):
                        ...
                        if root not in existing.split(":"):
                            os.environ["DYLD_FALLBACK_LIBRARY_PATH"] = (
                                f"{root}:{existing}" if existing else root
                            )
                            logger.debug(
                                "Added cache fallback DYLD_FALLBACK_LIBRARY_PATH: %s",
                                root,
                            )
                        break
...
def _build_emulator_command(
    ...
    *,
    ...
    if system == "Darwin":
        if not no_bundle:
            xemu_path, portable_mode_config_path = _build_macos_xemu_binary_paths(
                xemu_path
            )
    elif system == "Linux":
        if xemu_path.endswith("AppImage"):
            # AppImages need to have the xemu.toml file within their home dir.
            portable_mode_config_path = os.path.join(
                f"{xemu_path}.home", ".local", "share", "xemu", "xemu"
            )
    elif system == "Windows":
        pass
    else:
...
def _determine_output_directory(
    ...
    try:
        ...
        logger.exception(
            "Failed to run emulator command to determine output directory: %s", stderr
        )
        raise
...
def _process_arguments_and_run() -> int:
    ...
    xemu = (
        os.path.abspath(os.path.expanduser(args.xemu))
        if args.xemu
        else _download_xemu(cache_path, args.xemu_tag)
    )
    if not xemu:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                _generate_xemu_toml(
                    ...
                    hdd_path=args.hdd
                    if args.hdd
                    else os.path.join(temp_dir, "hdd.img"),
                    memory=args.memory,
...
def merge_main() -> int:
    ...
    parser.add_argument(
        "--output-dir", "-o", required=False, help="Directory to store merged results."
    )
    parser.add_argument(
        "--emit-schema",
        "--schema",
